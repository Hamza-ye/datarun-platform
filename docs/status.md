...
- Recommended next slice: no production-auth baseline fix remains. Choose the next product/platform direction: mobile OIDC login UX, an online binding-admin API/admin-authority decision, an IdP group/claim provisioning-input decision, or returning to NW-036 entity lifecycle/trigger/reporting expansion. Do not treat IdP claims as direct platform authority without a successor decision.
