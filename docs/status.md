...
| **2b: Multi-Actor Scope + Supervisor** | **Complete** | All 8 quality gates pass. 58 server tests (51 prior + 7 Phase 2b). 22 mobile tests (20 prior + 2). |
...
- `server/sync/` — Two-Tx pipeline (TransactionTemplate: Tx1 persist, Tx2 CD + flags). Pull: scope-filtered with post-query activity + subject_list filtering (AND within assignment, OR across assignments)
...
1. Phase 2c: Authorization Flags + Sync Hardening
   - 3 new flag categories: `scope_violation`, `role_stale`, `temporal_authority_expired`
   - Authorization CD in push pipeline
   - Selective-retain on mobile (scope contraction)
   - Auth mechanism hardening (Keycloak decision point)
