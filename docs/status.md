...
**Last updated**: 2026-06-05 (NW-051 special read/write access route accepted)
...
- IDR-028 (Production Principal-Binding Administration) is active. It selects deployment-managed manifest provisioning for production binding administration, defers online admin APIs, and NW-040 implemented the selected provisioning/audit path.
...
- Landed implementation slices: Phase 4.1 role-action enforcement from IDR-021/IDR-023, including authoritative server `role_stale` action-authority semantics and mobile advisory role-action gating; Phase 4.2 flag severity defaults and deployment-wide L0 `flag_severity_overrides` from IDR-022; IDR-024 / FP-007 assignment administration hardening; FP-008 assignment command identity binding; Phase 4.3 domain uniqueness schema, detector, and mobile advisory uniqueness from IDR-022; Phase 4.4 platform pattern registry and activity binding validation from IDR-020; IDR-025 pattern definition contract/package delivery; Phase 4.5 rebuildable pattern-state projection for enabled bindings including `ongoing_resolution/v1`; FP-005 subject-history backfill; FP-009 runtime resolver designation and single-writer resolution enforcement; Phase 4.6 transition detection; P04 scenario-grade Responsibility Binding coverage; FP-010 platform payload shape contract parity; NW-037/IDR-027 production-auth principal binding foundation; NW-038 OIDC/JWKS auth-provider validation; NW-040 production principal-binding provisioning; and NW-050/IDR-029 assignment-admin command-capability enforcement. These do not implement auto-resolution, resolver reassignment, normal sync backfill, durable workflow-state tables, mobile OIDC login UX, online production binding-admin APIs, group/claim authority, or access-exception runtime behavior.
- Recommended next slices: use NW-052 for shared-device actor-session lifecycle; NW-053 for subject/query/custom scope pressure under BAR-108; NW-054 for device data expiry/retained local data under BAR-106. If a concrete product/security need promotes broad audit/history access or emergency override writes after NW-051, create a bounded successor decision route before implementation. Use NW-044 through NW-046 if the next move should instead explore reporting aggregation/import-export, domain conflict automation/batch resolution, or generic flag cascade/pattern traversal reporting. Use the NW-047 companion before concrete operational UI, reporting, workflow, or product-design work. Do not treat IdP claims, activity role maps, scenario domain terms, or UX vocabulary as new authority without a successor decision.
...
| IDR-029: Assignment-Admin Command Capability | **ACTIVE** | [idr-029-assignment-admin-command-capability.md](decisions/idr-029-assignment-admin-command-capability.md) |
