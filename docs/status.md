...
**Last updated**: 2026-06-04 (NW-039 principal-binding administration decision routed)
...
- FP-011 remains open for production principal-binding administration/provisioning and any group/claim authority model. NW-037 and NW-038 did not make groups, roles, JWT `actor_id`, or IdP claims direct platform authority.
- NW-039 is the next ready slice: decide the production principal-binding administration/provisioning path. Use `docs/agent-working-surface/prompts/NW-039-decide-production-principal-binding-administration.md`; recommended first path is deployment-managed binding provisioning, not an online admin API.
- No baseline acceptance candidate is currently active. NW-040 is blocked on the NW-039 decision; entity lifecycle, trigger/reporting expansion, auto-resolution, resolver reassignment, and new scope mechanisms remain future-decision follow-ups.
...
- IDR-027 (Production Auth Principal-To-Actor Binding) is active. It defines explicit principal binding, forbids group/claim/JWT `actor_id` direct authority, and now includes the NW-038 OIDC/JWKS validation mode; binding administration/provisioning remains a later FP-011/BAR-104 gate.
...
- Landed implementation slices: Phase 4.1 role-action enforcement from IDR-021/IDR-023, including authoritative server `role_stale` action-authority semantics and mobile advisory role-action gating; Phase 4.2 flag severity defaults and deployment-wide `flag_severity_overrides` from IDR-022; IDR-024 / FP-007 assignment administration hardening; FP-008 assignment command identity binding; Phase 4.3 domain uniqueness schema, detector, and mobile advisory uniqueness from IDR-022; Phase 4.4 platform pattern registry and activity binding validation from IDR-020; IDR-025 pattern definition contract/package delivery; Phase 4.5 rebuildable pattern-state projection for enabled bindings including `ongoing_resolution/v1`; FP-005 subject-history backfill; FP-009 runtime resolver designation and single-writer resolution enforcement; Phase 4.6 transition detection; P04 scenario-grade Responsibility Binding coverage; FP-010 platform payload shape contract parity; NW-037/IDR-027 production-auth principal binding foundation; and NW-038 OIDC/JWKS auth-provider validation. These do not implement auto-resolution, resolver reassignment, normal sync backfill, durable workflow-state tables, binding administration/provisioning, mobile OIDC login UX, or group/claim authority.
- Recommended next slice: NW-039 production principal-binding administration path decision. Keep it focused on how `(issuer, subject) -> actor_id` bindings are created, rotated, deactivated, audited, and bootstrapped; prefer deployment-managed binding provisioning for the first implementation route unless product explicitly needs online administration now. Do not add group/claim authority, new scope mechanisms, resolver reassignment, auto-resolution, or envelope changes.
