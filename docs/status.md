...
- No baseline acceptance candidate is currently active. NW-042 accepted the constrained S22 coordinated distribution campaign runtime probe without adding new platform primitives; use the NW-042 backlog row for exact evidence. NW-047 is the next recommended stewardship slice before serious operational UI implementation. NW-043 now has a bounded exploration handoff for access exceptions and assignment-admin command authority; run it after NW-047 when UX/product vocabulary may affect the recommendation, and keep it exploration-only until it routes a successor decision/probe/implementation slice. NW-044 through NW-046 remain future-decision exploration routes for reporting aggregation/import-export, domain conflict automation/batch resolution, and generic flag cascade/pattern traversal reporting. Entity lifecycle, trigger/reporting expansion, auto-resolution, resolver reassignment, online production binding-admin APIs, and new scope mechanisms remain future-decision follow-ups.
