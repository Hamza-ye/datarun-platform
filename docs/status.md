...
- Recommended next slice: no production-auth baseline fix remains. Use NW-042 if the next move should be runtime validation: a constrained S22 coordinated distribution campaign probe that reuses current assignment, subject-history, transfer-pattern, flag, projection, and reporting semantics. Use NW-043 through NW-046 if the next move should be exploration before implementation: access exceptions/assignment-admin authority, reporting aggregation/import-export, domain conflict automation/batch resolution, or generic flag cascade/pattern traversal reporting. Do not treat IdP claims, activity role maps, or scenario domain terms as new authority without a successor decision.
