...
**Ship-3 close-out Wave 2-B (2026-04-27)**: one row demoted under the newly-adopted [Standing Rule R-7](../flagged-positions.md#rule-r-7-simplementation-parity) (§S–implementation parity). `field_count_budget` STABLE → DEFERRED: ADR-004 §S13 is exercised-partial (Wave-1 G-9 commit [`7540fc2`](../../server/src/main/java/dev/datarun/ship1/event/ShapePayloadValidator.java) added the boot-loop guard + the callable `validateShapeBudget` shim, but the HTTP push path does not invoke it per-event yet). Promotion back to STABLE is gated on closure of [FP-012b](../flagged-positions.md#fp-012b--adr-004-s13-http-runtime-field-count-budget-enforcement). No other rows demoted: a conservative R-7 sweep against the parity walk (`docs/reviews/system/adr-005-parity.md` and Ship retros) found no further STABLE rows whose §S is unambiguously `decided-unexercised`; ambiguous candidates (e.g., `pattern` ADR-009 §S3, `conflict-resolved` ADR-007 §S2 — no resolution emission yet) are left AS-IS pending orchestrator review per Wave 2-B's conservative-sweep guidance. No ADR drafted, no classification changed for any other row. Counts: **STABLE 257, DEFERRED 8, OBSOLETE 11, OPEN 0, PROPOSED 0, DISPUTED 0**. Total 276.

**Ship-3 close-out Wave 3/W4 (2026-04-27)**: ADR-006-R fully superseded ADR-006 while carrying §S1–§S4 forward and adding §S5 alias-cycle prevention. One new STABLE row added: `cycle_violation` (FLAG, ADR-006-R §S5), catalog row 9, `manual_only`, emitted by the push-path cycle guard. [FP-019](../flagged-positions.md#fp-019--alias-cycle-guard-implementation-tracking) is RESOLVED: implementation commit `a582e59`, tests commit `0b14607`, and `AliasCycleGuardTest` covers both persisted-graph and in-flight batch cycle closure. Carried ADR-006 rows are re-cited to ADR-006-R per the supersession rule; no classification changes. Counts: **STABLE 258, DEFERRED 8, OBSOLETE 11, OPEN 0, PROPOSED 0, DISPUTED 0**. Total 277.
...
|---|---|---|---|---|---|---|
| accept-and-flag | INVARIANT | ADR-006-R §S1 | STABLE | ADR-002 | round 0: DISPUTED (A:ALGORITHM, B:INVARIANT, C1:ALGORITHM, C2:ALGORITHM); round 1: INVARIANT (ADR-006 §S1); round 2: STABLE (quiet-scan promotion — no upstream ADR churn, no classification change); Ship-3 closeout Wave 3/W4: re-cited to ADR-006-R §S1 (full supersede, carried forward) | Property vs. procedure split: property is INVARIANT (this row), procedure lives at `conflict-detection` (ALGORITHM). Canonical over prior ADR-002 S14 cite. |
...
| concurrent-state-change | FLAG | ADR-006-R §S2 | STABLE | ADR-002 S7 | round 0: FLAG (inventory); round 1: class-membership cited (ADR-006 §S2); round 2: STABLE (quiet-scan promotion — no upstream ADR churn, no classification change); Ship-3 closeout Wave 3/W4: re-cited to ADR-006-R §S2 | structural conflict |
...
| conflict-detection | ALGORITHM | ADR-006-R §S3 | STABLE | ADR-002 | round 0: ALGORITHM (inventory); round 1: ALGORITHM confirmed (ADR-006 §S3); round 2: STABLE (quiet-scan promotion — no upstream ADR churn, no classification change); Ship-3 closeout Wave 3/W4: re-cited to ADR-006-R §S3 | Procedure that enforces the §S1 invariant. Extensible/relocatable without changing the invariant. |
...
| cycle_violation | FLAG | ADR-006-R §S5 | STABLE | ADR-006-R / contracts/flag-catalog.md / FP-019 | Ship-3 closeout Wave 3/W4 (2026-04-27): CREATED — FLAG (ADR-006-R §S5); FP-019 exercised-met; implementation commit `a582e59`, tests commit `0b14607` | Alias-cycle closure flag. Push-path batch-serial guard emits `conflict_detected/v1` with `payload.flag_category = "cycle_violation"` and `actor_ref = "system:cycle_guard/cycle_violation"`. Catalog row 9; `manual_only`; implemented by `CycleGuard` and asserted by `AliasCycleGuardTest`. |
| domain-uniqueness-violation | FLAG | ADR-006-R §S2 | DEFERRED | ADR-004 Session 3 Part 4 | round 0: FLAG (inventory); round 1: class-membership cited (ADR-006 §S2); Ship-3 closeout Wave 3/W4: re-cited to ADR-006-R §S2 | flag category (entry 7); emission deferred to Phase 4 per flag-catalog |
...
| flag | INVARIANT | ADR-006-R §S2 | STABLE | ADR-001 | round 0: DISPUTED (B:DERIVED, C1:INVARIANT, C2:PRIMITIVE); round 1: INVARIANT (ADR-006 §S2); round 2: STABLE (quiet-scan promotion — no upstream ADR churn, no classification change); Ship-3 closeout Wave 3/W4: re-cited to ADR-006-R §S2 | Event-stream canonicality (scoped, not global): on-stream anomaly records take flag form with no parallel record-surface; off-stream surfaces (metrics/telemetry/logs) are out of scope. Instances are DERIVED; catalog members are CONFIG. |
...
| flag-creation-location | INVARIANT | ADR-006-R §S4 | STABLE | raw-B | round 0: OPEN (inventory); round 1: INVARIANT (ADR-006 §S4); round 2: STABLE (quiet-scan promotion — no upstream ADR churn, no classification change); Ship-3 closeout Wave 3/W4: re-cited to ADR-006-R §S4 | Server-side by default; additively evolvable to device. Canonicalizes `architecture/boundary.md §SG-1`. |
...
| role-stale | FLAG | ADR-006-R §S2 | STABLE | ADR-003 S7 | round 0: FLAG (inventory); round 1: class-membership cited (ADR-006 §S2); round 2: STABLE (quiet-scan promotion — no upstream ADR churn, no classification change); Ship-3 closeout Wave 3/W4: re-cited to ADR-006-R §S2 | role changed flag; canonical over `role-staleness` (OBSOLETE) |
| role-staleness | OBSOLETE | ADR-006-R §Consequences | OBSOLETE | ADR-003 S7 | round 0: FLAG (inventory); round 1: OBSOLETE (ADR-006 §Consequences); Ship-3 closeout Wave 3/W4: re-cited to ADR-006-R (full supersede) | Synonym of `role-stale` — deduped in ADR-006. Row retained for traceability per ledger rule 5. |
...
| scope-violation | FLAG | ADR-006-R §S2 | STABLE | ADR-003 S7 | round 0: FLAG (inventory); round 1: class-membership cited (ADR-006 §S2); round 2: STABLE (quiet-scan promotion — no upstream ADR churn, no classification change); Ship-3 closeout Wave 3/W4: re-cited to ADR-006-R §S2 | unauthorized access flag category |
...
| stale-reference | FLAG | ADR-006-R §S2 | STABLE | ADR-002 S8 | round 0: FLAG (inventory); round 1: class-membership cited (ADR-006 §S2); round 2: STABLE (quiet-scan promotion — no upstream ADR churn, no classification change); Ship-3 closeout Wave 3/W4: re-cited to ADR-006-R §S2 | entity update lag |
...
| temporal-authority-expired | FLAG | ADR-006-R §S2 | STABLE | ADR-003 S7 | round 0: FLAG (inventory); round 1: class-membership cited (ADR-006 §S2); round 2: STABLE (quiet-scan promotion — no upstream ADR churn, no classification change); Ship-3 closeout Wave 3/W4: re-cited to ADR-006-R §S2 | assignment ended |
...
| transition-violation | FLAG | ADR-006-R §S2 | DEFERRED | ADR-005 Session 1 | round 0: FLAG (inventory); round 1: class-membership cited (ADR-006 §S2); Ship-3 closeout Wave 3/W4: re-cited to ADR-006-R §S2 | invalid state transition; emission deferred to Phase 4 per flag-catalog |
