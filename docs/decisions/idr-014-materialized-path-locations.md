...
`events.location_path` is aligned only if treated as immutable write-time infrastructure metadata for each event. Location reparenting must not rewrite historical event `location_path` values. Future events use the current resolved path. Controlled backfill is allowed only for events that had no `location_path` at insertion because the subject location was not yet known.
...
- Path may be recomputed for the mutable reference tables if a location is reparented (`locations` + `subject_locations`), but historical `events.location_path` values must not be recomputed or cascade-updated.
- Future subject events use the current resolved path at insert time. Backfill of `events.location_path` is allowed only for previously `NULL` event rows whose subject location was unknown at insertion, and must be a controlled admin operation.
