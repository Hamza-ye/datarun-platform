...
- **W-6 — Additive happy path (mandatory).** Two devices on `household_observation/v1`; server's shape registry is updated to include v2; both devices then capture under v2. Server validates and persists. **`ConflictDetector` flags `identity_conflict` for a v1-current capture whose prior duplicate (same `village_ref`, same normalized `household_name`, different `subject_id`) was recorded under v2.** The detector's current entry guard pins activation to `shape_ref = household_observation/v1`; the **v2-current direction is intentionally not asserted here** and folds into [FP-012](../flagged-positions.md#fp-012--deployer-authoring-surface-for-shapestriggerspolicies) gate (b)/(c) when shape-declared uniqueness lands. The **FP-009 closure assertion** stands on this directional reading: the detector is unchanged across Ship-3 and fires across mixed v1/v2 events in the v1-current direction because the identity-key field names (`village_ref`, `household_name`) are preserved per §6.1 sub-decision 2. Exercises [ADR-004 §S10](../adrs/adr-004-configuration-boundary.md#s10-shape-definition-versioning-and-evolution) (additive evolution) + [ADR-001 §S2](../adrs/adr-001-offline-data-model.md#s2-write-granularity) (multi-version projection on demand).
...
14. **FP-009 closure asserted (v1-current direction).** [`ConflictDetector`](../../server/src/main/java/dev/datarun/ship1/integrity/ConflictDetector.java) is **unchanged** across Ship-3 (no edits to the `detect(...)` payload field-name reads — `village_ref` and `household_name` remain the literal field names looked up). W-6 passes against a mixed v1+v2 event corpus, firing `identity_conflict` for a v1-current capture whose prior duplicate was recorded under v2. **Asymmetry note**: the detector's entry guard at [`ConflictDetector.java`](../../server/src/main/java/dev/datarun/ship1/integrity/ConflictDetector.java) line 35 (`HOUSEHOLD_SHAPE = "household_observation/v1"`) pins activation to v1-current captures; the v2-current direction of mixed-version detection is **deliberately not asserted** by W-6 and folds into [FP-012](../flagged-positions.md#fp-012--deployer-authoring-surface-for-shapestriggerspolicies) gate (b)/(c) when shape-declared uniqueness lands. The field-name-coupling concern that opened FP-009 is closed by this row (identity-key field names preserved across v1/v2 per §6.1 sub-decision 2); the version-pinning expedient is a separate FP-009-shaped surface that does not require a new FP because it is already covered by FP-012's gate. If this row fails, FP-009 stays OPEN.
