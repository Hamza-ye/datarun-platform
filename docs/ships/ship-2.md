...
| **R2 — Lineage DAG enforcement under race** | [ADR-002 §S9](../adrs/adr-002-identity-conflict.md#s9-lineage-graph-acyclicity-by-construction) | **Carried forward, not stressed in Ship-2's slice.** §S10 (online-only, server-validated transaction) is the intended guard against concurrent-coordinator races. Ship-2 runs a single coordinator on a single JVM — the in-practice guard is single-writer + Postgres MVCC + PK/UNIQUE constraints, not a constructed concurrent-coordinator load. The first Ship that runs >1 coordinator simultaneously (Ship-5 judgment / multi-resolver, certainly later) owes the race walkthrough. R2 carries forward as a Ship-5-or-later risk; not exercised here. **Spec-language correction (2026-04-26)**: prior wording "§S10 … puts under load for the first time" overstated coverage and was downgraded after Session-3 pressure-test surfaced the gap. |
...
**Setup** (preceding all walkthroughs): bootstrap with Ship-1's CHV-A (village-1) + CHV-B (village-2) plus the new `coordinator` actor token. Ship-2 needs two captures that fire `identity_conflict` (same normalized `household_name` + same village, different `subject_id`). Implementer may pick the duplicate-construction strategy:

- **Strategy A** (cross-CHV, dual-flag): CHV-A captures "Khan" in village-1 (subject `S_X`). CHV-B captures "Khan" in village-1 (subject `S_Y`). CHV-B writing into village-1 also fires `scope_violation` because village-1 is outside CHV-B's active scope; W-3 consumes only `identity_conflict`. The `scope_violation` is incidental and accepted (Ship-1 W-2 mechanism unchanged). Document the dual-flag state in a test comment so it is not mistaken for drift.
- **Strategy B** (single-CHV, clean): CHV-A captures "Khan" in village-1 twice with different `subject_id`s. Single `identity_conflict`, no `scope_violation` noise. No bootstrap change required.

Either strategy is acceptable for W-3/W-4/W-5; merge/split semantics are independent of how the duplicate was detected. Both captures persist (accept-and-flag); subjects `S_X` and `S_Y` both exist in the registry as Ship-1 leaves them.
