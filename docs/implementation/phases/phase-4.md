...
- [x] Server and mobile pattern projections only re-admit flagged source events for canonical accepted resolutions; `reclassified` remains identity-conflict subject-attribution behavior unless a later decision broadens pattern projection semantics.
...
- [x] Resolving a flag as accepted makes the source event eligible for projection again, while legal pattern transitions still govern state advancement.
...
- **2026-05-25**: FP-010 platform payload shape contract parity landed as post-Phase 4 contract hygiene: server runtime bundles all six `contracts/shapes/*.schema.json` files, validates platform payloads during sync push and event insertion, pins actual assignment/conflict/identity emission paths to those contracts, removes platform payload schema ownership from deployer shape registry bootstrap, filters legacy mirror rows out of config packages, and moves `ongoing_resolution/v1` assignment transfer to platform-owned pattern `platform_shape_roles`. FP-011 remains open and out of scope until production authentication/OIDC work is explicitly scheduled.
