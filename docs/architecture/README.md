...
| P3 | Records are append-only | Immutable events, alias-based merge, flag rather than reject structurally valid state anomalies |
| P4 | Patterns compose | One subject-level pattern per activity, event-level compose freely, cross-activity via `activity_ref` |
| P5 | Conflicts are surfaced | Accept-and-flag for state anomalies, detect-before-act, single-writer resolution |
...
| **Flag** | A state anomaly raised against a structurally valid event. Never causes state-based rejection. 9 categories |
...
| **Accept-and-flag** | The universal state-anomaly handling mechanism. Structurally valid events are accepted; anomalies are surfaced as flags |
