...
| O-5 | Subject-list scope for ongoing subjects | Existing `subject_list` scope type [4-S7]. Assigning responsibility for a long-running subject is an `AssignmentCreated` event with `subject_list` scope. | No new scope type — existing mechanism. |
