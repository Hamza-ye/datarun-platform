...
**R-7 §S–implementation parity** is now a per-Ship retro deliverable. Each Ship retro produces (or updates) `docs/reviews/system/adr-N-parity.md` for any ADR whose §S positions the Ship first-exercised, classifying each §S as `decided-unexercised` / `exercised-met` / `exercised-violated`. The Ship cannot tag with any `exercised-violated` rows open. Concept-ledger rows referencing a §S transition from `DEFERRED` to `STABLE` only when the §S is `exercised-met`. R-7 is the Standing Register Rule adopted at Ship-3 closeout (2026-04-27); see [`flagged-positions.md` Rule R-7](flagged-positions.md#rule-r-7-simplementation-parity) for the full rule text and the failure-mode evidence that motivated adoption.
