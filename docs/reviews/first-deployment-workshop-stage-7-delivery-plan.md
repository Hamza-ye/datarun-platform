...
| M1 Candidate 1 product/spec and UX validation | Define bounded operational capture over existing kernel: setup, assigned work, capture, local save, sync, correction, optional subject link, freshness, unresolved issue visibility. | Product/UX validation artifacts, S06 timing decision, explicit exclusions. |
...
| Lane | Dependency |
|---|---|
| Candidate 1 | Existing accepted kernel, UX/product validation, S06 timing decision, and Stage 8 packet gates. It must not depend on production auth, reporting, retention/security, or conflict automation. |
| S06/entity lifecycle | Product Manager priority plus BAR-105/S06 product/platform decision. Stop if maintained known things, lifecycle states, or discovered-unit stewardship are required before Candidate 1 freeze. |
...
| Before M1 freeze | S06 timing decision: Candidate 1 either excludes lifecycle with UX copy/tests, or S06/BAR-105 decision moves before Candidate 1 implementation planning. | Product Manager with steward accountability |
...
- Any S06 pressure inside Candidate 1 triggers the explicit S06 timing decision
  before implementation planning continues.
...
- Hold the S06 timing decision before Candidate 1 packet freeze.
