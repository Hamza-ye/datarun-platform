...
- Carry PREOP-001 and PREOP-002 into atomization planning.
...
### PREOP-001: Deployment And Tenancy Guardrail
...
- Event-envelope atoms must preserve the current envelope and reject tenant/deployment fields unless formal change control reopens the envelope.
...
- Any future deployment/ops atom must state that tenant isolation is not yet a closed platform mechanism.
...
Reopen before deployment/ops atomization if self-host packaging, data residency, or managed cloud operations require a stronger deployment identity contract.

### PREOP-002: Authentication And Actor Mapping Guardrail
...
- Authorization/sync atoms must preserve assignment-derived access and sync scope as access scope.
- Event-envelope atoms must preserve `actor_ref` as authorship and `device_id` as device/app-installation identity.
- Admin/configuration atoms may later discuss groups as provisioning convenience, but must not make groups authority sources.
...
Reopen before authorization/sync atomization if a proposed spec would let account, group, or identity-provider claims directly grant event access without assignment-derived authority.
