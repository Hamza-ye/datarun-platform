...
1. Validate `07-system-boundary-map.md` against this control overlay.
2. Keep the first identity atomized spec limited to reference contract plus subject-lineage contract.
3. Put actor/assignment authority in the authorization/sync spec.
4. Put process identity and pending match in workflow/process-pattern spec work.
5. Put conflict resolution lifecycle in flag/resolution spec work.
