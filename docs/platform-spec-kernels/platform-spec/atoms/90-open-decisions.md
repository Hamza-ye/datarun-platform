...
| Final reference serialization and active emission sites | Platform-spec detail gap | Event Envelope / Schema plus Identity / Lineage, Assignment / Authority / Sync, Configuration, Projection / Workflow State, and owning behavior atoms | The accepted baseline requires typed references, but canonical field names, placement, cardinality, and required emission sites for accepted subject, causal, process, assignment, or other typed-reference values remain open. Do not infer universal source-event fields, central pre-registration, or referent lifecycle ownership from reference vocabulary. | A later atom or implementation needs canonical reference serialization, field placement, cardinality, or required emission sites. |
| Event correlation and grouping semantics | Hold-back / platform-spec detail gap | Event Envelope / Schema for `activity_ref` field meaning; Configuration for activity instances; Projection / Workflow State and owning behavior atoms for workflow, process, campaign, distribution, review, reporting, or cross-flow grouping semantics; Assignment / Authority / Sync where grouping affects visibility or authority | `activity_ref` remains optional configured activity context. Event correlation, grouping, workflow membership, process identity, campaign/distribution grouping, review grouping, cross-flow linking, and reporting grouping semantics are not closed. Do not treat `activity_ref` as a universal grouping key, pattern identity, workflow/process identity, product work-item identity, authority scope, or substitute for typed references, payload/source links, pattern/projection contracts, assignment/scope, or later accepted grouping semantics. | Before SPEC-009, SPEC-010, SPEC-011, SPEC-012, a selected implementation slice, or an implementation design uses `activity_ref` or activity context to group events, derive workflow/process state, drive pattern membership, correlate events, scope visibility/authority, traverse source chains, or aggregate reports. |
| Platform-bundled shape inventory | Platform-spec detail gap | Owning behavior atoms plus Event Envelope / Schema and Configuration | Shapes are deployer-defined by default, and platform-bundled shapes are narrow exceptions for explicitly platform-owned facts. Final inventory for identity, integrity/conflict, registration, review/resolution, task, or other platform-owned facts remains open. Do not treat bundled shapes as a general platform-owned domain schema catalog or new envelope `type` values. | SPEC-004, SPEC-005, SPEC-006, SPEC-009, SPEC-010, or implementation design needs a normative platform-bundled shape. |
...
| Assessment visibility | Platform-spec / operational policy gap | Assignment / Authority / Sync plus Reporting / Aggregation | Assessment visibility semantics are not closed. Do not infer assessment visibility from role labels, reporting views, product surfaces, or hierarchy labels by implication. | Assessment views require visibility beyond current assignment/scope mechanisms or become part of a product slice. |
...
| Permission table and activity/context authority details | Platform-spec / operational policy gap | Assignment / Authority / Sync plus Configuration | Activity and context may be inputs to authority projection through assignment, role, scope, and configuration mechanisms, but exact permission tables, role/action mappings, and activity/context authority details remain open. Do not treat `activity_ref` as an authority grant or complete authority context. | SPEC-007 or implementation design needs concrete permission matrices, role/action bindings, or normative activity/context authority semantics. |
...
| Onboarding and role-transition details | Operational policy plus implementation/tooling gap | Assignment / Authority / Sync plus Configuration and Local Data Lifecycle | Onboarding, responsibility transfer, actor setup, role transition, and handoff policy are not closed. Do not derive fixed role-transition workflow or local lifecycle effects from assignment-derived authority by implication. | Onboarding, responsibility transfer, actor setup, or role transition behavior must become normative. |
...
| Flag event identity, creation location, and resolution-event mapping | Architecture / platform-spec decision gap | Flag / Resolution plus Event Envelope / Schema | Final flag identity, canonical flag creation location, server-created flag default, and resolution-event type mapping are not closed. | `SPEC-010` or an implementation design needs normative flag creation, flag identity, or resolution event mapping. |
...
| Configuration versioning and stale-configuration reconciliation | Platform-spec / implementation tooling gap | Configuration plus Event Envelope / Schema and Assignment / Authority / Sync | Atomic configuration packages, shape/version coexistence, and in-progress offline work under older configuration remain unresolved. Do not retroactively invalidate structurally valid offline work solely because newer configuration has synced. | SPEC-006, SPEC-004, or implementation design defines configuration package versioning, stale-configuration reconciliation, or validation of offline work created under older configuration. |
...
| Auto-resolution authoring and monitoring | Implementation/tooling gap | Flag / Resolution plus Reporting / Aggregation | ADR-005 closes eligible workflow auto-resolution attribution, but authoring UX and monitoring/reporting surfaces remain open. Do not extend auto-resolution to non-workflow or security-relevant conflicts without formal classification. | SPEC-010 or implementation design needs auto-resolution policy authoring, audit, monitoring, or reporting behavior. |
