...
Recommendation: keep the current containment-only model accepted for the current baseline, but route a successor IDR before exposing or expanding production assignment-admin UI/API behavior. Split access exceptions into separate successor exploration because auditor access, shared devices, grace periods, subject-scope variants, and new scope mechanisms are not the same problem as assignment create/end command authority.
