...
| Deployment packaging and operations | The source has a Dockerfile, dev compose, Flyway migrations, and environment properties, but no verified clean production image. | NW-063 found that the current `server/` Docker context omits required root contract resources from the packaged JAR; BAR tests prove runtime behavior from the repository build, not the clean image. | NW-063 inspection and isolated clean build; current Docker/compose assets remain non-production evidence. | `operator_deployable_with_constraints` at source/kernel level; deployable image and turnkey ops `not_started`. | Correct image resource packaging, TLS/reverse proxy, secrets, backup/restore, observability, migration/rollback runbook, and one reference deployment profile. | Follow NW-063 through NW-067; do not treat dev compose or the current Dockerfile as production hardening. |
...
| Observability/backup/upgrade readiness | `no` | Dockerfile, Flyway, and dev compose exist, but NW-063 found the clean image omits required root contract resources. There is also no production deployment profile, monitoring/alerting, backup/restore, disaster recovery, migration rollback, capacity, or SLO runbook. |
