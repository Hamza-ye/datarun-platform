...
| GAP-OPS-01 | Production deployment runbook and operations rehearsal | NW-063 map accepted; successors routed | Operational policy gap / platform-spec detail gap / implementation-tooling gap; architecture only if accepted semantics would change | NW-063 selected one application host behind external TLS with durable external PostgreSQL 16. Run NW-064 policy, NW-065 tooling, NW-066 runbook/plan, then NW-067 rehearsal. A clean current `server/` image build omits required root contract resources, so current Docker/compose assets are not deployment evidence. Do not claim stronger readiness before a passing rehearsal. |
