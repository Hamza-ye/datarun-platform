...
| GAP-CONFLICT-01 | UX/product evidence for flag queue ergonomics without semantic drift. | Conflict UI, flag queue, or stewardship workflow work becomes active. | none | Select NW-072 first if durable flag behavior is needed, then a bounded UI row. | Flag queue behavior is accepted or current semantics are extracted. | NW-072 | Platform spec or implementation evidence. |
| GAP-CONFLICT-02 | Product/platform evidence for automation or batch resolution. | Batch resolution, auto-resolution, conflict automation, or resolver reassignment pressure. | platform decision. | Select NW-045; select NW-072 first if current flag behavior must become normative. | Automation/batch policy is accepted or rejected. | NW-045 | Architecture decision route or platform spec. |
| GAP-CONFLICT-03 | Evidence for explicit resolver-steward eligibility. | Conflict/admin UX or production hardening depends on resolver eligibility beyond current fallback. | platform/product decision. | Select NW-072 for current behavior extraction, then a successor eligibility decision if needed. | Resolver eligibility policy is accepted or fallback is removed from product path. | NW-072 | Platform spec or architecture decision route. |
