...
| GAP-CONFLICT-01 | Flag queue ergonomics | PC2 current behavior accepted; bounded implementation/read UI route open only if selected | Platform-spec detail gap / implementation tooling gap | NW-072 accepted the PC2-scoped current flag behavior and operational attention query boundary. Remaining open work is a bounded PC2 implementation/read UI route only if selected; this does not imply queue, batch, automation, resolver reassignment, or broad conflict console behavior. |
...
| GAP-CONFLICT-01 | Accepted NW-072 platform spec plus bounded PC2 implementation/read UI evidence if selected. | PC2 one-item attention review/read UI work becomes selected. | none | Select a bounded PC2 implementation/read UI row using accepted NW-072; do not create queue, batch, automation, resolver reassignment, or broad console scope. | Bounded PC2 read UI is accepted or the route is withdrawn. | NW-072 accepted reference; successor not selected | Platform spec plus implementation evidence if selected. |
