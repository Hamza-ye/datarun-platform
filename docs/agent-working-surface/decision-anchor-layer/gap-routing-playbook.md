...
| GAP-OPS-01 | Production deployment runbook and operations rehearsal | Selected as NW-063 | Operational policy gap / platform-spec detail gap / implementation-tooling gap; architecture only if accepted semantics would change | Run NW-063 first to inventory assets, compare one reference deployment, classify decisions, and define runbook/rehearsal evidence. Route accepted procedures and tooling as separate successors. Do not treat development compose as production hardening or claim readiness before rehearsal. |
